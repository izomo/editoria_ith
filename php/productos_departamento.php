...
if (@!$_SESSION['user']) {
  header("Location:../index.php");
	...
	if(strlen($_GET['desde'])>0 and strlen($_GET['hasta'])>0) {
		...
		$pdf->SetFont('Arial', 'B', 11);
		$pdf->Cell(58, 38, '', 0);
		$pdf->Cell(5, 28, 'Reporte de Servicios Por Departamento', 0);
		$pdf->Ln(9);
		$pdf->Cell(60, 8, '', 0);
		...
		while($productos2 = mysql_fetch_array($productos)){
			...
			$pdf->Cell(17, 8, date('d/m/Y', strtotime($productos2['fecha'])), 0,0,'C');
	...
	//Parte Inferior
	$pdf->SetFont('Arial', 'B', 8);
	$pdf->Cell(175, 8, 'Total de Copias del Departamento', 1);
	$pdf->SetFont('Arial', '', 8);
